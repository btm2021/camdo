<template>
  <div>
    ...
    <b-modal size="lg" no-stacking ref="modal_taonhanh" id="modal_taonhanh" hide-footer hide-header>
      <div>
          ...
          <b-row>
            ...
            <b-col cols="6">
              <b-form-group label="Tên khách :" description="Tên khách mua">
                <b-form-input autocomplete="off" v-model="formHoaDonNhanh.tenkhach"
                  placeholder="Nhập tên khách"></b-form-input>
              </b-form-group>
              <b-form-group label="Căn Cước Công Dân Khách :" description="CCCD của khách">
                <b-form-input autocomplete="off" v-model="formHoaDonNhanh.cccd"></b-form-input>
              </b-form-group>
              <b-form-group label="Tổng tiền :" :description="$formatN(formHoaDonNhanh.tongtien)">
              ...
              <b-form-group label="Địa chỉ :" description="Địa chỉ khách">
                <b-form-input autocomplete="off" v-model="formHoaDonNhanh.diachi"></b-form-input>
              </b-form-group>
              <b-form-group label="Số điện thoại :" description="Số điện thoại khách">
                <b-form-input autocomplete="off" v-model="formHoaDonNhanh.sodienthoai"></b-form-input>
              </b-form-group>
            </b-col>
            <b-col cols="6">
              <b-form-group label="Trọng Lượng Vàng :" :description="$formatSoVang(parseFloat(formHoaDonNhanh.klv) * 1000).fullStr
                ">
                <b-form-input @change="changeTongSoTienFromHoaDonNhanh" type="range" step="0.1" min="0.3" max="50"
                  autocomplete="off" v-model="formHoaDonNhanh.klv" required></b-form-input>
              </b-form-group>
              <b-form-group label="Trọng Lượng Vàng :" :description="$formatSoVang(parseFloat(formHoaDonNhanh.klv) * 1000).fullStr
                ">
                <b-form-input @change="changeTongSoTienFromHoaDonNhanh" type="number" step="0.1" min="0.3" max="50"
                  autocomplete="off" v-model="formHoaDonNhanh.klv" required></b-form-input>
              </b-form-group>
              <b-form-group label="Loại Vàng :" description="Loại vàng : [9950]">
                <b-form-input autocomplete="off" disabled v-model="formHoaDonNhanh.id_banggia" required></b-form-input>
              </b-form-group>

              <b-form-group label="Ghi chú hóa đơn :" description="Ghi chú">
                <b-form-textarea v-model="formHoaDonNhanh.ghichu" autocomplete="off"></b-form-textarea>
              </b-form-group>
              <b-button type="submit" variant="primary">Tạo Hóa Đơn_ Thường</b-button>
        ...
        <b-navbar-nav>
          <b-nav-item-dropdown text="Tạo nhanh">
            <b-dropdown-group header="Số lượng sẵn vàng 9950">
              <b-dropdown-item href="#" @click="showModalTaoNhanhHoadon(0.3)">Tạo Nhẫn 3p</b-dropdown-item>
  ...
  methods: {
    ...
    async switch_in_camdo_onchange(item) {
      ...
      await this.$supabase
        ...
        .eq("id", item.id);
      this.$bvToast.toast(
        `Thay đổi tình trạng IN của gói cầm đồ [${item.invoice_number}].Từ [${!item.invoice_label ? "ĐÃ IN" : "CHƯA IN"
    ...
    async switch_chuoc_camdo_onchange(item) {
      ...
      await this.$supabase
        ...
        .eq("id", item.id);
      this.$bvToast.toast(
        `Thay đổi tình trạng CHUỘC của gói cầm đồ [${item.invoice_number
        }].Từ [${!item.invoice_status ? "ĐÃ CHUỘC" : "CHƯA CHUỘC"}] thành [${item.invoice_status ? "ĐÃ CHUỘC" : "CHƯA CHUỘC"
        }] thành công`,
        {
          title: "Thông báo",
          autoHideDelay: 3000,
