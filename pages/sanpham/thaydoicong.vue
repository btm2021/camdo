      ...
      <b-overlay :show="overlay_ipt">
        <b-input-group>
          ...
          <template #append>
            <b-dropdown
              :text="
                !action ? 'Hành Động' : action === 'cong' ? '+ Công' : '- Công'
              "
              variant="primary"
            >
              <b-dropdown-item @click="action = 'cong'"
                >Cộng (+) Thêm Công</b-dropdown-item
              >
              <b-dropdown-item @click="action = 'tru'"
                >Trừ (-) Bớt Công</b-dropdown-item
              >
            </b-dropdown>
  ...
  methods: {
    async setThayDoi() {
      if (this.action && this.congchange > 0) {
        let str = `Bạn muốn ${
          this.action === "cong" ? "Cộng thêm" : "Trừ Bớt"
        } ${this.$formatN(
          parseInt(this.congchange) * 1000
        )} Tiền Công của những món trên ?`;
        let x = confirm(str);
        if (x) {
    ...
    async doitiencong() {
      return new Promise(async (resolve, reject) => {
        for (let i = 0; i < this.listsanpham.length; i++) {
          ...
          if (this.action === "cong") {
            tiencongmoi =
              parseInt(this.listsanpham[i].cong || 0) +
              parseInt(this.congchange);
          }
          if (this.action === "tru") {
            tiencongmoi =
              parseInt(this.listsanpham[i].cong || 0) -
          ...
          await this.$supabase
            .from("sanpham")
            .update({ cong: tiencongmoi, product_intem: false })
            .eq("id", this.listsanpham[i].id);
        }
        resolve(true);
      });
    },
